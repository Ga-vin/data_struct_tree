#ifndef BINARY_TREE_TEST_H_
#define BINARY_TREE_TEST_H_

void T_init_bitree(void);

void T_destroy_bitree(void);

void T_create_node_bitree(void);

void T_clear_bitree(void);

void T_is_empty_bitree(void);

void T_depth_bitree(void);

void T_get_value_bitree(void);

void T_set_bitree(void);

void T_parent_bitree(void);

void T_left_child_bitree(void);

void T_right_child_bitree(void);

void T_insert_child_bitree(void);

void T_del_child_bitree(void);

void T_pre_order_traverse_bitree(void);

void T_in_order_traverse_bitree(void);

void T_post_order_traverse_bitree(void);

#endif /* BINARY_TREE_TEST_H_ */
